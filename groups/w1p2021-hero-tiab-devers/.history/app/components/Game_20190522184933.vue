    ...
    <div v-if="step.id > 2" class="game"> <!--TEMPLATE GAME AFTER STEP 2-->
      ...
      <div @dblclick="openInventory" class="inventory">
        ...
        <div class="inventory__list">
          <ul class="items">
            <li v-for="(item, index) in items" :key="index" :item="item" class="item"><img :src="step.item" alt="" class="itemsImg"><p class="itemsName">{{ item }}</p></li>
          </ul>
        </div>
      ...
      <img @dblclick="takeBuche" v-if="step.itemName === 'Buche'" class="itemsTake buche" :src="step.item" alt="Items Playing">
      <div class="menu">
...
export default {
  ...
  methods: {
    ...
    takeBuche() {
      ...
      let current = game.steps[this.subtitle];

      console.log("TA MERE " + current);

      if (step.subtitle === "Map 1.1") {
